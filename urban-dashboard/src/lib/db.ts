...
export function getHubsFromGpkg(city: string) {
  ...
  const tables = db.prepare(`SELECT table_name FROM gpkg_contents WHERE data_type = 'features'`).all() as any[];
  if (tables.length === 0) return { type: 'FeatureCollection', features: [] };
  
  const tableName = tables[0].table_name;
  const geomColRes = db.prepare(`SELECT column_name FROM gpkg_geometry_columns WHERE table_name = ?`).get(tableName) as any;
...
export function getLayerFromGpkg(city: string, fileName: string) {
  ...
  const features = rows.map(row => {
    ...
    if (row[geomCol]) {
      try {
        const buf = Buffer.from(row[geomCol]);
    ...
    delete row[geomCol];
    for (const key in row) if (typeof row[key] === 'bigint') row[key] = row[key].toString();
    return { type: 'Feature', geometry: geojson, properties: row };
  });
  return { type: 'FeatureCollection', features };
}

function hstoreToObject(hstoreStr: string | null) {
  if (!hstoreStr) return {};
...
export async function getHubDetails(city: string, lat: number, lon: number, hubId?: string) {
  ...
  if (hubId && fs.existsSync(dnaGpkg)) {
    ...
    try {
      const tables = s.prepare(`SELECT table_name FROM gpkg_contents WHERE data_type = 'features'`).all() as any[];
      if (tables.length > 0) {
         // Sortujemy na wszelki wypadek malejąco by wziąć największą grawitację
         const row = s.prepare(`SELECT * FROM "${tables[0].table_name}" WHERE hub_id = ? ORDER BY raw_gravity DESC LIMIT 1`).get(Number(hubId)) as any;
         if (row) {
           hubMetrics = row;
           // Sanityzacja bigint by nie wywaliło res.json w Next.js
           for (const key in hubMetrics) {
             if (typeof hubMetrics[key] === 'bigint') hubMetrics[key] = hubMetrics[key].toString();
           }
         }
      }
    } catch(e) {}
  ...
  const pop = popQ.map((p: any) => ({
    ...p,
    grid_id: p.grid_id ? p.grid_id.toString() : null,
    pop_val: p.pop_val ? p.pop_val.toString() : null
  }));

  const pois = pq.map((poi: any) => ({
    ...poi,
    poi_id: poi.poi_id ? poi.poi_id.toString() : null,
    w: Number(poi.w),
    sum_pull: Number(poi.sum_pull)
  }));
